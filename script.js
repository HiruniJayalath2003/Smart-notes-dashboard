...
function renderNotes(noteList) {//create funtion
    ...
    const color = noteList.level === "Important" ? "bg-red-500 text-white" :
        noteList.level === "Study" ? "bg-fuchsia-800 text-white" :
            noteList.level === "Work" ? "bg-lime-500" :
    ...
    <div class="noteCard border rounded-md p-5 border-gray-400">
     ...
     <div class="descBar flex items-center justify-between py-5">
      ...
      <p class="priority border rounded-[15px] px-1 py-0.5 border-gray-500 ${color}">${noteList.level}</p>
      </div>
     </div>
